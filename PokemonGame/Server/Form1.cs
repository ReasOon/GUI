...
namespace Server
{
    public partial class Form1 : Form
    {
        ...
        void WorkServer()
        {
            ...
            try
            {
                richTextBox1.Text = ("서버를 시작합니다.");
                socket.Bind(new IPEndPoint(IPAddress.Parse(txtIPAddress.Text), Convert.ToInt32(nudPort.Value)));
                socket.Listen(backlog);
            }
            catch
            {
                richTextBox1.Text = ("서버 시작에 실패하였습니다.");
                return;
            }

            richTextBox1.Text = ("서버가 정상적으로 구동되었습니다.");
            ...
            while (true)
            {
                ...
                new Thread(() =>
                {
                    ...
                    catch (Exception ex) { richTextBox1.Text += ("클라이언트 연결중 오류 발생 : " + ex.Message + ex.StackTrace); }
                }).Start();
        ...
        void ClientProcess(Socket client)
        {
            ...
            richTextBox1.Text += ("클라이언트가 접속했습니다. [" + client.RemoteEndPoint + "]");
            ...
            while (true)
            {
                ...
                else if (receiveData.Contains("LOGIN"))
                {
                    ...
                    {
                        richTextBox1.Text += (client.RemoteEndPoint + "의 닉네임은 설정되지않았습니다.");
                    }
                }
                //연결종료
                ...
                {
                    string chatMessage = receiveData.Split('|')[1];
                ...
                if (receiveData != "ping")
                {
                    try { richTextBox1.Text += (clientNickName + "[" + client.RemoteEndPoint + "] : " + receiveData); }
                    catch { }
                }
            }
        }
        ...
        void GameProcess()
        {
            ...
            do
            {
                ...
                //회차 증가하며 로그생성
                richTextBox1.Text += ("== " + Convert.ToString(++gameIndex) + "회차 게임생성 [" + Convert.ToString(gameEndTime) + "]");
                ...
                richTextBox1.Text += ("== " + Convert.ToString(gameIndex) + "회차 게임마감 [" + gameResult + "]");
